...
class RuleTests extends FlatSpec  {
  ...
  it should "match contains" in {
	...
	def checkSystem(rule: RuleSystem) {
		...
		intercept[RuleViolation] { rule.check(Checkable("scheisse in german shouldn't", "de")) }
	}

  "RuleSystem" should "work flat" in { checkSystem(orig) }
	it should "work combined" in { checkSystem(orig.combineRules) }
