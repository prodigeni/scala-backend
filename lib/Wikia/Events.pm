...
use constant LOCAL_USER_PATH => '/usr/wikia/backend/bin/scribe/events_local_users.pl';
use constant UPDATE_DATAWARE_PATH => '/usr/wikia/backend/bin/fixes/update_pages.pl';

has "host"   => (
	is			=> "rw",
	isa			=> "Str",
	required	=> 1,
	default		=> '10.10.10.153'
);
has "port"   => (
	is			=> "rw",
	isa			=> "Int",
...
has "queue"  => (
	...
	default 	=> sub {
		my $self = shift;
		return HttpSQS->new( $self->host, $self->port, 'tcp', 'utf-8');
	}
);
has "queue_id" => (
	is 			=> "rw",
...
sub job {
	...
	if ( ref( $qinfo ) ) {
		...
		do {
			# parse message
			$self->record( $self->queue->get( $qid ) );
			# check status
...
sub _update_dataware {
	...
	if ( !defined $self->row->{ cityId } ) {
		say "Invalid Wiki ID";
		return 0;
	}

	if ( !defined $self->row->{ pageId } ) {
		say "Invalid page ID";
		return 0;
	}
	...
	"/usr/bin/perl",
	"$Bin/../checkPID.pl --dry ",
	"--script=\"" . UPDATE_DATAWARE_PATH . " --cityid=" . $self->row->{cityId} . " --pageid=" . $self->row->{pageId} . " --silent $delete\""
	);
