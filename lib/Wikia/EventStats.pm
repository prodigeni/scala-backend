...
sub __make_stats($;$$$) {
	...
	$lb->yml( $Wikia::EventStats::YML ) if ($Wikia::EventStats::YML);
	
	# categories and languages 
	my $dbr = new Wikia::DB( {"dbh" => $lb->getConnection( Wikia::LB::DB_SLAVE, 'stats', Wikia::LB::EXTERNALSHARED )} );		
	my %wikis = ();		
	my @where = ( 'ccm.city_id = cl.city_id', 'l.lang_code = cl.city_lang' );
	my @options = ();
	my $sth = $dbr->select_many("ccm.city_id, ccm.cat_id, cl.city_lang, l.lang_id", "city_list cl, city_cat_mapping ccm, city_lang l", \@where, \@options);
	if ($sth) {
		while(my ($city_id, $cat_id, $city_lang, $lang_id) = $sth->fetchrow_array()) {
			%{$wikis{$city_id}} = ( 'cat' => $cat_id, 'lang' => $city_lang, 'langid' => $lang_id );
		}
		$sth->finish();
	}
	...
	foreach my $num ( ($self->params->{type} eq CAT_LANG_TYPE ) ? sort keys %{$databases} : sort ( map { sprintf("%012u",$_) } ( keys %{$databases} ) ) ) {
		...
		if ( $self->params->{type} eq WIKIA_TYPE ) {
			...
			my $cat = $wikia->{'cat'};
			# language
			my $lang = $wikia->{'langid'};
			print "Category for Wiki: $cat, language for Wiki: " . $wikia->{'lang'} . " ($lang) \n";
			if ( $cat && $lang) {
				#
...
sub __row_stats($;$$) {
	...
	if ( $table ne '' ) {
		...
		if ( $dbw ) {
			...
			my $q = $dbw->delete($table, \@conditions);
			...
			if ( $key_field ) {
				if ( UNIVERSAL::isa($key_field, 'HASH') ) {
					foreach my $column ( keys %$key_field ) {
						$data{"$column"} = Wikia::Utils->intval($key_field->{$column}); 
					}
				} else {				
					$data{"$key_field"}	= Wikia::Utils->intval($row->{id});
				}
			}
			
			$ins = $dbw->insert( $table, "", \%data );
...
sub __articles_new($$$;$) {
	...
	if ( defined($stats) ) {
		...
		if ( $self->params->{type} eq WIKIA_TYPE ) { #$row->{month} < '201009' &&
