...
GetOptions('master|m=s' => \$MASTER,
           'solr-version|V=s' => \$VERSION);
...
sub start_worker {
	...
	if ( $curr_workers <= $WORKERS && $num_jobs > 0 ) {
		...
		my @program;
		push @program, '/usr/bin/perl';
		push @program, '/usr/wikia/backend/bin/solr-worker.pl';
		push @program, '--master='.$MASTER if $MASTER;
		push @program, '--solr-version='.$VERSION if $VERSION;
		push @program, $tmp;
		push @program, '-v';
...
sub get_files {
	...
	@files = grep { $_ =~ /\d{4}-\d{2}-\d{2}_\d{5}/ && $_ ne $curr_file } @files;
