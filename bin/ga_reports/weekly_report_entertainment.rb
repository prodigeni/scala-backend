...
@subject = 'Entertainment Weekly Google Analytics report'
@wikia_users = [ 'ga-weekly-entertainment-analytics-report@wikia-inc.com' ]
...
Dir.mktmpdir("weekly-stats", "/tmp") do |tmpdir|
  FasterCSV.open("#{tmpdir}/wikis.csv", "w") do |csv|
    ...
    results.sort.each do |city_id,city|
      if city[:lang] == 'en' && city[:cat] == "Entertainment"
        pageviews = []
