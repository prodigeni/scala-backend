...
@results = {}
@dates = []
@wikia_users = ['Danny Horn <danny@wikia-inc.com>',
                'Douglas Kennedy <douglas@wikia-inc.com>',
                'Adam Carey <adam@wikia-inc.com>',
                'Gil Penchina <gil@wikia-inc.com>',
                'Jeska <jeska@wikia-inc.com>',
                'Kim <kimberly@wikia-inc.com>',
                'Joe <joe@wikia-inc.com>',
                'Mike Schwartz <mike@wikia-inc.com>',
                'Janicke Eckbo <janicke@wikia-inc.com>',
                'Wikia International <international-l@wikia-inc.com>',
...
Dir.mktmpdir("monthly-stats", "/tmp") do |tmpdir|
  FasterCSV.open("#{tmpdir}/wikis.csv", "w") do |csv|
    ...
    csv << ["City_ID", "DBName", "Category", "URL", "Lang",  @dates].flatten
    results.sort.each do |city_id,city|
      ...
      csv << [ city_id, city[:dbname], city[:cat], city[:url], city[:lang], pageviews ].flatten
    end  
  end
  Dir.chdir(tmpdir)
