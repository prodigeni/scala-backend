...
sub rec_exists($$;$) {
	...
	$type = 0 unless $type;
	my $dbx = new Wikia::DB( {"dbh" => $lb->getConnection( Wikia::LB::DB_MASTER, undef, Wikia::LB::STATS )} );
	
	my @options = ();
	my @where = ( 
		"wiki_id = " . $dbx->quote($row->{city_id}),
		"page_id = " . $dbx->quote($row->{page_id})
	);
	if ( $type && $type == 3 ) {
		push @where, "event_type = 3";
	} else {
		push @where, "rev_id = " . $dbx->quote($row->{rev_id});
		push @where, "log_id = " . $dbx->quote($row->{log_id});
	}
	my $oRow = $dbx->select(
		" count(0) as cnt ",
		" events ",
...
sub fetch_data($;$) {
	...
	my $q = "SELECT " . join( ',', @db_fields ) . " from events where wiki_id = '".$city_id."' and rev_timestamp between '".$start_date."' and '".$end_date."' order by page_id" ;
...
sub check_records($$) {
	my ( $self, $city_id, $dbname, $start_date, $end_date ) = @_;
	...
	if ($sth) {
		while(my $values = $sth->fetchrow_hashref()) {
			...
			if ( $exists ) {
				...
				my $sql = $dbw->insert('scribe_events', 0, $rows, $options, 1);
				print "New record added to scribe_events: " . Dumper($rows) . "\n";
				$all_fixed++;
	...
	if ($sth) {
		while(my $values = $sth->fetchrow_hashref()) {		
			...
			if ( $exists || $count_exists ) {
				...
				my $options = undef;
				my $dbw = new Wikia::DB( {"dbh" => $lb->getConnection( Wikia::LB::DB_MASTER, undef, Wikia::LB::STATS )} );
				print "New archive record added to scribe_events: " . Dumper($rows) . "\n";
				my $sql = $dbw->insert('scribe_events', 0, $rows, $options, 1);
				$all_fixed++;
				$dbw->disconnect if ($dbw);
				$dbr->disconnect if ($dbr);
...
=cut					
					...
					if ( 
						...
						if ( $revision->{userid} ne $row->{'user_id'} ) {
							...
							push @$differences, "user_id => new: " . $revision->{userid} . ", old: " . $row->{'user_id'};
						}
						
						if ( $page->{namespace} ne $row->{'page_ns'} ) {
							$data{'page_ns'} = $page->{namespace};
							push @$differences, "page_ns => new: " . $page->{namespace} . ", old: " . $row->{'page_ns'};
						}
						
						if ( $is_content ne $row->{'is_content'} ) {
							$data{'is_content'} = $is_content;
							push @$differences, "content => new: " . $is_content . ", old: " . $row->{'is_content'};
						}
						
						if ( $is_redirect ne $row->{'is_redirect'} ) {
							$data{'is_redirect'} = $is_redirect;
							push @$differences, "redirect => new: " . $is_redirect . ", old: " . $row->{'is_redirect'};
						}
						
						if ( $revision->{timestamp} ne $row->{'rev_timestamp'} ) {
							$data{'rev_timestamp'} = $revision->{timestamp};
							push @$differences, "timestamp => new: " . $revision->{timestamp} . ", old: " . $row->{'rev_timestamp'};
						}					
						...
						if ( !$dry) {
							if ( scalar keys %data ) {
								print "Update data for Wiki: " . $wiki->{id} . " and page: " . $page->{id} . " and $uid: $id_value \n";
								print "Differences: \n\t" . join("\n\t", @$differences) . "\n";
				...
				if ( $notfound ) {
					...
					if ( !$count_exists ) {
						...
						if ( !$dry) {
							print "Delete data for Wiki: " . $row->{wiki_id} . " and page: " . $row->{page_id} . " and $uid: $id_value \n";
							$dbs->delete('events', $where);
							$all_changed++;
						} else {
...
my $dates = Wikia::Utils->between_dates_days_ago($days);
my $start_date = $dates->[0];
my $end_date = $dates->[1];
print "Fetch data (last $days days): between $start_date and $end_date \n";
...
print "get list of wikis from city list \n";
my ($databases) = $dbr->get_wikis(\@where_db);
$dbr->disconnect() if ( $dbr );

my $dbs = $lb->getConnection( Wikia::LB::DB_SLAVE, undef, Wikia::LB::STATS );
foreach my $num (sort ( map { sprintf("%012u",$_) } (keys %{$databases}) ))
{
	...
	# check page & revision
	print "Check page & revision \n";
	my $result = $oEStats->check_records($city_id, $databases->{$city_id}, $start_date, $end_date);
	print "page & revision: " . $result->{ok} . ", " . $result->{invalid} ." \n";
	
	# check archive 
	print "Check archive data \n";	
	$result = $oEStats->check_archive($city_id, $databases->{$city_id}, $start_date, $end_date);
...
print "New events: $all_fixed, changed: $all_changed\n";
